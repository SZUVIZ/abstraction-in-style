...
def save_contours_as_single_png(contour1, contour2, filename):
    # Find the min/max coordinates across all contour points.
    ...
    # Shift contour coordinates so they fit inside the image.
...
def find_closest_pair(contour1,contour2):
        ...
        for outer_point in contour1:
            for inner_point in contour2:
                dist = np.linalg.norm(outer_point[0] - inner_point[0])  # Compute the distance
...
def connect_mask_interior_exterior(mask: Union[np.ndarray,torch.Tensor]):
    ...
    if len(contours)>1:
        for contou in sorted_contours[1:]:
            ...
            cv2.line(mask, closest_pair[0], closest_pair[1], (0), 3)  # Connect the contours
    return mask
