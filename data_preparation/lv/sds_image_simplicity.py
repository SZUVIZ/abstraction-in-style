...
@torch.no_grad()
def get_text_embeddings(device, pipe: Any, text: str) -> T:
...
@torch.no_grad()
def decode(latent: T, pipe: Any, im_cat: TN = None):
    image = pipe.vae.decode((1 / 0.18215) * latent, return_dict=False)[0]
    image = denormalize(image)
    return image

def init_pipe(device, dtype, unet, scheduler) -> Tuple[Any, T, T]:
...
class SDSLoss:
    ...
    def __init__(self, device, pipe: Any, dtype=torch.float32):
        self.t_min = 50
        self.t_max = 950
        self.alpha_exp = 0
        self.sigma_exp = 0
...
def image_optimization(device, pipe: Any, image: np.ndarray, text_target: str, num_iters: int = 200,):
    ...
    with tqdm(total=num_iters, desc="Processing image", unit="value") as pbar:   
        for i in range(num_iters):
            loss, _ = sds_loss.get_sds_loss(z_taregt, embedding_target, guidance_scale=0)
            optimizer.zero_grad()
            (2000 * loss).backward()
            optimizer.step()
...
def sds_based_simplification(device, image: str, simp_img_seq_indexs: List[int], simp_img_seq_save_path: str, all_simp_img_seq_save_path: str = "-1"):
    ...
    if str(device) == "cpu" or not torch.cuda.is_available():
        raise RuntimeError(
            "SDS simplification requires CUDA, but torch.cuda.is_available() is False in the current environment. "
            "Check your PyTorch/CUDA/driver compatibility."
        )
