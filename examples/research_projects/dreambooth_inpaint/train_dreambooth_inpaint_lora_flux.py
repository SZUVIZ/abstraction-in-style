...
def get_mask(im_shape, original_image_path, mask_data_path):
    if os.path.isdir(mask_data_path):
    ...
    elif os.path.isfile(mask_data_path):
        mask_path = mask_data_path
    else:
        raise ValueError(f"Mask data path {mask_data_path} is not a valid directory or file.")
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " resolution. Can be a single integer (e.g. '512') or a tuple of (width, height) (e.g. '1024,512')"
        ),
    )
    parser.add_argument(
        "--center_crop",
...
def parse_resolution(resolution_str):
    ...
    try:
        ...
        raise ValueError(
            f"Resolution must be a single integer (e.g. '512') or a tuple of (width, height) (e.g. '1024,512'). Got {resolution_str}"
        )
    ...
    def __init__(
        ...
        if args.dataset_name is not None:
            ...
            for ext in image_extensions:
                image_paths.extend(list(self.instance_data_root.glob(f"*{ext}")))
                image_paths.extend(list(self.instance_data_root.glob(f"*{ext.upper()}")))
            
            self.instance_images_path = sorted(image_paths)
            
            # Find corresponding text files with same basename
            self.custom_instance_prompts = []
            instance_images = []
                ...
                if text_path.exists():
                    try:
                        with open(text_path, "r", encoding="utf-8") as f:
                            prompt = f.read().strip()
                        self.custom_instance_prompts.append(prompt)
                        instance_images.append(Image.open(img_path))
                    except Exception as e:
                        logger.warning(f"Skipping {img_path}: {e}")
                else:
                    logger.warning(f"No matching text file found for {img_path}, using default prompt")
                    self.custom_instance_prompts.append(self.instance_prompt)
                    instance_images.append(Image.open(img_path))
        ...
        self.pixel_values = []
        # Only resize, no cropping or flipping
        ...
        # Only resize, no cropping for image_transforms
        ...
        # Only resize for image_transforms_resize
...
def main(args):
    # Disallow using --report_to=wandb and --hub_token together for security reasons.
    if args.report_to == "wandb" and args.hub_token is not None:
        raise ValueError(
            "You cannot use both --report_to=wandb and --hub_token due to a security risk of exposing your token."
            " Please use `huggingface-cli login` to authenticate with the Hub."
        )
    # Also raise an error because MPS does not support bf16.
    ...
    """Create the Accelerator, configure precision and project state, disable AMP on MPS, and then set logging and seed."""
    ...
    # Parse --resolution into (W, H), or make it square if a single value is provided.
    # Parse resolution into tuple
    args.resolution = parse_resolution(args.resolution)
    logger.info(f"Using resolution: {args.resolution}")


    # Generate class images if prior preservation is enabled.
    """Generate class images before DreamBooth training when prior preservation is enabled."""
    """If --with_prior_preservation is enabled, the script samples extra class images with the pretrained FluxFillPipeline until --num_class_images is reached."""
    """The default is False."""
    # 1. instance images, e.g. "a photo of sks dog"
    # 2. class images, e.g. "a photo of dog"
    # Using only instance images may cause language drift and overfitting.
    # With prior preservation, the loss becomes:
    # Loss = instance_loss + lambda * prior_loss
    if args.with_prior_preservation:
    ...
    # Create the Hub repository on the main process if --push_to_hub is enabled.
    ...
    # Example config.json structure for FluxFillPipeline.
    # {
    ...
    #   ]
    # }
    """Load the two tokenizers, text encoders, scheduler, VAE, and transformer, then freeze the base modules so only LoRA adapters are trained."""
    """Text is processed by the tokenizer before the text encoder."""
    # Load the tokenizers
    ...
    """Import the text encoder classes."""
    ...
    """Choose the base weight dtype from accelerator.mixed_precision, move models to accelerator.device, and enable checkpointing when requested."""
    ...
    """Build the LoRA config from --lora_layers or the default layer list, then attach LoRA adapters and collect the trainable parameters."""
    ...
    """Insert LoRA adapters into the transformer."""
    transformer.add_adapter(transformer_lora_config)

    # If requested, also add LoRA adapters to the text encoder (text_encoder_one only).
    if args.train_text_encoder:
    ...
    def load_model_hook(models, input_dir):
        ...
        """Load LoRA weights into transformer_ and, if enabled, also into text_encoder_one_, while warning about incompatible keys."""
    ...
    # Register save/load hooks so accelerator.save_state / load_state can serialize and restore LoRA weights correctly.
    ...
    """Pass LoRA parameters to the optimizer and optionally use a separate learning rate for the text encoder."""
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        if accelerator.is_main_process:
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                ...
                for i, image in enumerate(images):
                    image.save(os.path.join(args.output_dir, f"validation_image_{i}_{epoch}.png"))
